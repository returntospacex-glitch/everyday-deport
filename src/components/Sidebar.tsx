        ...
        <div className="w-72 h-screen glass border-r border-white/10 flex flex-col p-8 fixed left-0 top-0">
            ...
            <nav className="flex-1 space-y-2">
                {menuItems.map((item) => {
                    ...
                    return (
                        ...
                        >
                            <item.icon className={clsx("w-6 h-6", isActive ? "text-accent" : "group-hover:text-white")} />
            ...
            {user && (
                <div className="pt-4 border-t border-white/10 flex items-center justify-between gap-2">
                    <button
                        onClick={() => logout()}
                        className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-xl text-white/30 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200 group"
                    >
