...
export default function Dashboard() {
    ...
    const [categories, setCategories] = useState(["습관", "건강", "학습"]);
    ...
    useEffect(() => {
        ...
        const unsubCategories = onSnapshot(categoriesRef, (docSnap) => {
            if (docSnap.exists()) {
                setCategories(docSnap.data().list || ["습관", "건강", "학습"]);
            } else {
                setCategories(["습관", "건강", "학습"]);
            }
            setIsLoaded(true);
        });
    ...
    const saveCategoriesToFirestore = async (newList: string[]) => {
        if (!user) return;
        const db = getFirebaseDb();
                            ...
                            {categories.map(cat => (
                                <div key={cat} className="group relative">
                                    <button
                                        ...
                                        className={`
                                            px-5 py-2.5 rounded-full text-base font-bold transition-all border
                                            ${activeCategory === cat
                            ...
                            {isAddingCategory ? (
                                ...
                                >
                                    <input
                                        ...
                                        className="bg-transparent text-sm font-bold text-white focus:outline-none w-20"
                                    />
                                    <button
                                        onClick={() => {
                                            if (newCategoryName && !categories.includes(newCategoryName)) {
                                    ...
                                    <div className="flex items-center gap-4 overflow-hidden flex-1" onClick={() => openEditModal(routine)}>
                                        ...
                                        <div className="truncate">
                                            ...
                                            <div className="flex items-center gap-2 mt-1">
                                                <span className="text-[10px] px-1.5 py-0.5 rounded bg-white/5 text-white/40 font-bold">
                                                    {routine.category}
                                                </span>
                                                {routine.time && (
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar px-1">
                                            {categories.map(cat => (
                                                <button
                                                    key={cat}
                                                    onClick={() => setNewRoutine(prev => ({ ...prev, category: cat }))}
                                                    className={`
